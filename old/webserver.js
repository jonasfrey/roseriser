...
const PARSED_DXF_DIR = "./parsed_dxf";
...
async function handleRequest(request) {
    ...
    // API endpoint for saving parsed DXF with metadata
    if (path === "/api/save-parsed-dxf" && request.method === "POST") {
        try {
            let o_body = await request.json();
            let { s_name, s_type, s_id__linked_profile, o_sketch } = o_body;

            if (!s_name || !s_type || !o_sketch) {
                return new Response(JSON.stringify({ error: "Missing s_name, s_type, or o_sketch" }), {
                    status: 400,
                    headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" },
                });
            }

            await Deno.mkdir(PARSED_DXF_DIR, { recursive: true });

            let s_path__file = `${PARSED_DXF_DIR}/${s_name}.json`;
            let o_data = { s_name, s_type, s_id__linked_profile: s_id__linked_profile || null, o_sketch };
            await Deno.writeTextFile(s_path__file, JSON.stringify(o_data));

            console.log(`Saved parsed DXF: ${s_path__file} (type: ${s_type})`);

            return new Response(JSON.stringify({ b_success: true, s_name }), {
                status: 200,
                headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" },
            });
        } catch (e) {
    ...
    // API endpoint for listing all parsed DXFs (metadata only, no sketch data)
    if (path === "/api/list-parsed-dxf" && request.method === "GET") {
        try {
            await Deno.mkdir(PARSED_DXF_DIR, { recursive: true });

            let a_o_entry = [];
            for await (let o_entry of Deno.readDir(PARSED_DXF_DIR)) {
                if (o_entry.isFile && o_entry.name.endsWith(".json")) {
                    let s_path__file = `${PARSED_DXF_DIR}/${o_entry.name}`;
                    let s_content = await Deno.readTextFile(s_path__file);
                    let o_data = JSON.parse(s_content);
                    a_o_entry.push({
                        s_name: o_data.s_name,
                        s_type: o_data.s_type,
                        s_id__linked_profile: o_data.s_id__linked_profile || null
                    });
                }
            }

            return new Response(JSON.stringify(a_o_entry), {
                status: 200,
                headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" },
            });
        } catch (e) {
    ...
    // API endpoint for getting a specific parsed DXF (full data with sketch)
    if (path === "/api/get-parsed-dxf" && request.method === "GET") {
        try {
            let s_name = url.searchParams.get("name");
            if (!s_name) {
            ...
            return new Response(JSON.stringify(o_data), {
                status: 200,
                headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" },
            });
        } catch (e) {
    ...
    // API endpoint for deleting a parsed DXF
    if (path === "/api/delete-parsed-dxf" && request.method === "DELETE") {
        try {
            let s_name = url.searchParams.get("name");
            if (!s_name) {
                return new Response(JSON.stringify({ error: "Missing name parameter" }), {
            ...
            let s_path__file = `${PARSED_DXF_DIR}/${s_name}.json`;
            await Deno.remove(s_path__file);

            console.log(`Deleted parsed DXF: ${s_path__file}`);
