// Roseriser Button Generator Application
// Creates ornamental buttons from DXF patterns
...
let f_s_scad_from_a_o_sketch = function(o_sketch__ornament, s_name__ornament, o_sketch__profile, s_name__profile, o_sketch__remover, s_name__remover) {
    return f_s_scad_button_generator(
        o_sketch__ornament, s_name__ornament,
        o_sketch__profile, s_name__profile,
        o_sketch__remover, s_name__remover,
        32  // n_segment for arc/circle sampling
    );
};

// Calculate bounding box and ornament info
let f_o_button_info_from_o_sketch = function(o_sketch) {
    ...
    for(let o_entity of a_o_entity){
        if(o_entity.s_type === 'LINE') n_cnt__line++;
        if(o_entity.s_type === 'ARC') n_cnt__arc++;
        if(o_entity.s_type === 'CIRCLE') n_cnt__circle++;
    ...
    let n_radius = Math.max(n_scl_x, n_scl_y) / 2;
    ...
    return {
        ...
        n_cnt__arc,
        n_cnt__circle,
        n_cnt__total: a_o_entity.length
    };
};
...
let o_viewer__three = {
    ...
    init() {
        ...
        let o_helper__grid = new THREE.GridHelper(100, 10, 0x444444, 0x333333);
        o_helper__grid.rotation.x = Math.PI / 2;
        this.scene.add(o_helper__grid);

        let o_helper__axis = new THREE.AxesHelper(30);
...
let o_state = {
    o_file: { ornament: null, profile: null, remover: null },
    o_sketch: { ornament: null, profile: null, remover: null },
...
let o_viewer__ornament = f_o_viewer_svg('svg-container-ornament', 'svg-wrapper-ornament', 'header-ornament');
...
let f_init_viewer_svg = function() {
    o_viewer__ornament.init();
    o_viewer__profile.init();

    document.getElementById('zoom-in').addEventListener('click', () => {
        o_viewer__ornament.n_scl *= 1.2; o_viewer__ornament.updateTransform();
    ...
    document.getElementById('zoom-reset').addEventListener('click', () => {
        o_viewer__ornament.reset();
        o_viewer__profile.reset();
    });
    document.getElementById('zoom-fit').addEventListener('click', () => {
        o_viewer__ornament.fitToContainer();
    ...
    return new Promise((resolve) => {
        ...
        require(['vs/editor/editor.main'], function () {
            o_state.editor = monaco.editor.create(document.getElementById('editor-container'), {
                value: '// Upload DXF files to generate button SCAD output',
                language: 'cpp',
                theme: 'vs-dark',
...
let f_update_button_info = function() {
    let el_info = document.getElementById('button-info');
    if (!o_state.o_sketch.ornament) {
        el_info.style.display = 'none';
        return;
    }

    let o_info = f_o_button_info_from_o_sketch(o_state.o_sketch.ornament);
    document.getElementById('info-radius').textContent = o_info.n_radius.toFixed(2) + ' mm';
    document.getElementById('info-entity').textContent = `${o_info.n_cnt__line}L / ${o_info.n_cnt__arc}A / ${o_info.n_cnt__circle}C`;
...
let f_process_all_file = function() {
    ...
    if (o_state.o_sketch.ornament) {
        let s_svg = f_s_svg_from_o_sketch(o_state.o_sketch.ornament);
        o_viewer__ornament.setSVG(s_svg);
        o_state.o_output.svg = s_svg;
        document.getElementById('dl-svg').disabled = false;
        f_update_button_info();
    }
    ...
    // Generate JSON
    let o_data__json = {};
    if (o_state.o_sketch.ornament) o_data__json.ornament = o_state.o_sketch.ornament;
    ...
    document.getElementById('dl-json').disabled = !o_state.o_sketch.ornament;

    // Generate SCAD if all three files are loaded
    if (o_state.o_sketch.ornament && o_state.o_sketch.profile && o_state.o_sketch.remover) {
        let s_name__ornament = o_state.o_file.ornament.name.replace(/\.dxf$/i, '');
        ...
        o_state.o_output.scad = f_s_scad_from_a_o_sketch(
            o_state.o_sketch.ornament, s_name__ornament,
        ...
        f_update_status('All files processed - Button SCAD generated (15mm, 20mm, 25mm)');
        f_save_scad_to_server(o_state.o_output.scad, 'dxf-to-buttons');
    } else if (o_state.o_sketch.ornament && o_state.o_sketch.profile) {
        let s_name__profile = o_state.o_file.profile.name.replace(/\.dxf$/i, '');
    ...
    } else if (o_state.o_sketch.ornament) {
        o_state.o_output.scad = '// Upload profile and profile remover DXF files to generate full button SCAD output';
        f_update_status('Ornament loaded - upload profile files');
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    let el_select__ornament = document.getElementById('select-ornament');
    ...
    await Promise.all([
        f_populate_file_select(el_select__ornament),
    ...
    let el_file__ornament = document.getElementById('file-ornament');
    ...
    el_select__ornament.addEventListener('change', () => f_handle_select_change('ornament', el_select__ornament, el_file__ornament));
    ...
    el_file__ornament.addEventListener('change', (evt) => {
        if (evt.target.files[0]) f_handle_custom_file_upload('ornament', evt.target.files[0], el_select__ornament);
    });
    ...
    document.getElementById('dl-svg').addEventListener('click', () => {
        let s_name = o_state.o_file.ornament?.name.replace(/\.dxf$/i, '') || 'ornament';
        f_download_file(o_state.o_output.svg, `${s_name}.svg`, 'image/svg+xml');
    });
    document.getElementById('dl-scad').addEventListener('click', () => {
        let s_name = o_state.o_file.ornament?.name.replace(/\.dxf$/i, '') || 'output';
        f_download_file(o_state.o_output.scad, `${s_name}_buttons.scad`, 'text/plain');
    });
    document.getElementById('dl-json').addEventListener('click', () => {
        let s_name = o_state.o_file.ornament?.name.replace(/\.dxf$/i, '') || 'data';
        f_download_file(o_state.o_output.json, `${s_name}.json`, 'application/json');
    });

    f_update_status('Ready - select an ornament pattern to generate buttons');
