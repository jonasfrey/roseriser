...
line_2 = [[3.075752516391778, 23.4591354503404], [22.44632970541717, 0]];
                ...
                245.12552321637776  // end angle (degrees)
                        ...
                        arc_2 = [
                [25.5061173320571, 2.526514778403466, 0],  // center
                3.968069712798655,  // radius
                219.5470233809095,  // start angle (degrees)
                408.68288514705483  // end angle (degrees)
            ];
...
k3_tangent_point_0 = [22.966645, 41.094820, 0.000000];
k3_tangent_point_0_angle = 41.560872;
k3_tangent_point_1 = [0.000000, 41.464217, 0.000000];
k3_tangent_point_1_angle = 90.000000;
k3_tangent_point_2 = [0.000000, 0.000000, 0.000000];
k3_tangent_point_2_angle = 90.000000;
k3_tangent_point_3 = [22.446330, -0.000000, 0.000000];
k3_tangent_point_3_angle = -50.452977;

// ===== UNCONNECTED ENDPOINTS =====
// Points where entities have no connection (line/arc endpoints), with outward rotation angle
k3_endpoint_0 = [27.377820, 29.470365, 0.000000];
k3_endpoint_0_angle = 180.000000;
k3_endpoint_1 = [-9.859086, 35.168093, 0.000000];
k3_endpoint_1_angle = 335.125523;
k3_endpoint_2 = [28.125940, 5.506801, 0.000000];
k3_endpoint_2_angle = 498.682885;
k3_endpoint_3 = [-9.509212, 5.837391, 0.000000];
k3_endpoint_3_angle = 26.911227;
k3_endpoint_4 = [-0.000000, 20.732109, 0.000000];
k3_endpoint_4_angle = 221.560872;
k3_endpoint_5 = [3.075753, 23.459135, 0.000000];
k3_endpoint_5_angle = 129.547023;
...
module k3_place_revolve_joints_at_tangent_points(profile_for_revolve, profile_height, angle=90) {
    translate(k3_tangent_point_0)
    rotate([0, 0, k3_tangent_point_0_angle])
    revolve_profile_around_x(profile_for_revolve, profile_height, angle);
    translate(k3_tangent_point_1)
    rotate([0, 0, k3_tangent_point_1_angle])
    revolve_profile_around_x(profile_for_revolve, profile_height, angle);
    translate(k3_tangent_point_2)
    rotate([0, 0, k3_tangent_point_2_angle])
    revolve_profile_around_x(profile_for_revolve, profile_height, angle);
    translate(k3_tangent_point_3)
    rotate([0, 0, k3_tangent_point_3_angle])
    revolve_profile_around_x(profile_for_revolve, profile_height, angle);
}

// Module to place revolve joints at unconnected endpoints
module k3_place_revolve_joints_at_endpoints(profile_for_revolve, profile_height, angle=180) {
    translate(k3_endpoint_0)
    rotate([0, 0, k3_endpoint_0_angle])
    revolve_profile_around_x(profile_for_revolve, profile_height, angle);
    translate(k3_endpoint_1)
    rotate([0, 0, k3_endpoint_1_angle])
    revolve_profile_around_x(profile_for_revolve, profile_height, angle);
    translate(k3_endpoint_2)
    rotate([0, 0, k3_endpoint_2_angle])
    revolve_profile_around_x(profile_for_revolve, profile_height, angle);
    translate(k3_endpoint_3)
    rotate([0, 0, k3_endpoint_3_angle])
    revolve_profile_around_x(profile_for_revolve, profile_height, angle);
    translate(k3_endpoint_4)
    rotate([0, 0, k3_endpoint_4_angle])
    revolve_profile_around_x(profile_for_revolve, profile_height, angle);
    translate(k3_endpoint_5)
    rotate([0, 0, k3_endpoint_5_angle])
    revolve_profile_around_x(profile_for_revolve, profile_height, angle);
}
...
module k3_place_non_tangent_joints(profile) {

}

// Sweep pattern - sweeps profile along each path (lines, arcs, and circles)
module k3_sweep_pattern(profile) {
    union() {
        // Sweep lines
...
module k3_full_pattern(
    ...
    union() {
        k3_sweep_pattern(sweep_profile);
        if(b_make_joints){
            k3_place_revolve_joints_at_tangent_points(joint_profile_for_revolve, joint_profile_height, joint_angle);
        }
        if(b_make_endpoint_joints){
            k3_place_revolve_joints_at_endpoints(joint_profile_for_revolve, joint_profile_height, endpoint_joint_angle);
        }
        if(b_make_non_tangent_joints){
            k3_place_non_tangent_joints(sweep_profile);
        }
    }
}
...
//k3_sweep_pattern(profile_default(scalefactor=0.2));
