...
// Profile: p_4
// Points: 7 (xpositive), 11 (mirrored)
...
line_extension_amount = 20; // mm for lines
...
module test_sweep_full_pattern(
    ...
    sweep_profile = p_4_mirroredx,
    joint_profile_for_revolve = p_4_for_revolve,
    joint_profile_height = p_4_height,
    joint_angle = 90,
...
module part_with_difference(s=1){
    difference(){
        ...
        test_sweep_full_pattern(
            ...
            b_make_endpoint_joints=true,
            b_make_non_tangent_joints=true,
            sweep_profile=p_4_mirroredx_scaled(s=s),
            joint_profile_for_revolve=p_4_for_revolve_scaled(s=s),
            joint_profile_height=p_4_height * s,
            joint_angle=90,
            endpoint_joint_angle=180
        );

        color([1.0,0.0, 0.0, 0.5])
        translate([0, 0, pr_4_trn_y*s])
        test_sweep_full_pattern(
            b_make_joints=false,
            b_make_endpoint_joints=false,
            b_make_non_tangent_joints=true,
            sweep_profile=pr_4_mirroredx_scaled(s=s)
        );
    }
}
