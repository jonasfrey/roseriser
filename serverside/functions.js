...
import { f_o_result__upload_dxf, f_s_scad__generate_simple, f_s_scad__generate_simple_joints, f_s_scad__generate_simple_joints_remover, f_s_scad__generate_profile_revolve } from './dxf_functions.js';
...
// DXF upload handler: parse DXF content, store file, save to DB
o_wsmsg__upload_dxf.f_v_server_implementation = async function(o_wsmsg){
    let { s_dxf_content, s_name, s_type } = o_wsmsg.v_data;
    let o_result = await f_o_result__upload_dxf(s_dxf_content, s_name, s_type);

    // store in database via syncdata
    let s_name_table = f_s_name_table__from_o_model(o_model__o_dxffile);
    let o_dxffile = o_wsmsg__syncdata.f_v_sync({
        s_name_table,
        s_operation: 'create',
        o_data: {
            s_name: o_result.s_name,
            s_type: o_result.s_type,
            s_path_file: o_result.s_path_file,
            s_json_a_o_entity: o_result.s_json_a_o_entity,
        }
    });

    return {
        o_dxffile,
        n_cnt_entity: o_result.n_cnt_entity,
    };
};
...
o_wsmsg__generate_scad.f_v_server_implementation = async function(o_wsmsg){
    let { s_generation_type, n_id__profile, n_id__profile_remover, n_id__path, n_point_per_mm, s_sweep_function, b_mirror_side_override, b_flip_y, b_endpoint_revolves, b_round_mode, n_cylinder_radius, n_iter_x, n_iter_y, n_dist_x, n_dist_y } = o_wsmsg.v_data;
    ...
    b_flip_y = b_flip_y || false;
    b_endpoint_revolves = b_endpoint_revolves !== false;
    b_round_mode = b_round_mode || false;
    n_cylinder_radius = n_cylinder_radius || 0;
    n_iter_x = Math.max(1, parseInt(n_iter_x) || 1);
    n_iter_y = Math.max(1, parseInt(n_iter_y) || 1);
    n_dist_x = (n_dist_x === undefined || n_dist_x === null) ? 100 : Number(n_dist_x);
    n_dist_y = (n_dist_y === undefined || n_dist_y === null) ? 100 : Number(n_dist_y);
    let s_name_table = f_s_name_table__from_o_model(o_model__o_dxffile);

    let f_get = function(n_id){
        let a_o = o_wsmsg__syncdata.f_v_sync({
            s_name_table,
            s_operation: 'read',
            o_data: { n_id }
        });
        if(!a_o || a_o.length === 0){
            throw new Error(`DXF file with n_id=${n_id} not found`);
        }
        return a_o[0];
    };

    let o_dxffile__profile = f_get(n_id__profile);

    let s_scad = '';

    if(s_generation_type === 'profile_revolve'){
        s_scad = f_s_scad__generate_profile_revolve(o_dxffile__profile, n_point_per_mm, b_mirror_side_override, b_flip_y);
    } else if(s_generation_type === 'simple'){
        s_scad = f_s_scad__generate_simple(o_dxffile__profile, f_get(n_id__path), n_point_per_mm, false, s_sweep_function, b_mirror_side_override, b_flip_y, b_round_mode, n_cylinder_radius, n_iter_x, n_iter_y, n_dist_x, n_dist_y);
    } else if(s_generation_type === 'simple_endpoints'){
        s_scad = f_s_scad__generate_simple(o_dxffile__profile, f_get(n_id__path), n_point_per_mm, true, s_sweep_function, b_mirror_side_override, b_flip_y, b_round_mode, n_cylinder_radius, n_iter_x, n_iter_y, n_dist_x, n_dist_y);
    } else if(s_generation_type === 'simple_endpoints_joints'){
        s_scad = f_s_scad__generate_simple_joints(o_dxffile__profile, f_get(n_id__path), n_point_per_mm, s_sweep_function, b_mirror_side_override, b_flip_y, b_round_mode, n_cylinder_radius, b_endpoint_revolves, n_iter_x, n_iter_y, n_dist_x, n_dist_y);
    } else if(s_generation_type === 'simple_endpoints_joints_remover'){
        s_scad = f_s_scad__generate_simple_joints_remover(o_dxffile__profile, f_get(n_id__profile_remover), f_get(n_id__path), n_point_per_mm, s_sweep_function, b_mirror_side_override, b_flip_y, b_round_mode, n_cylinder_radius, b_endpoint_revolves, n_iter_x, n_iter_y, n_dist_x, n_dist_y);
    }

    // save generated SCAD file to disk
    let s_dir__scad = `${import.meta.dirname}/../.gitignored/scad_generated`;
    try { await Deno.mkdir(s_dir__scad, { recursive: true }); } catch { /* exists */ }
    let s_ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    let s_path_scad = `${s_dir__scad}/dxf2scad_${s_ts}.scad`;
    await Deno.writeTextFile(s_path_scad, s_scad);
